...
class Progress {
    ...
    toMB(byteLength) {
        const MB = 1024 * 1024;
        return (byteLength / MB).toFixed(2);
    }
    tick(count = 1) {
        let prependedHeader = '';
        ...
        if (this.currentProgress >= this.total) {
            if (this.throbber.hasActiveSpinners()) {
                ...
                // this.throbber.update(this.title, { text: `${this.title} ${this.progressType}ed in ${totalSec} seconds` });
        ...
        this.currentProgressFrame =
            `${this.title} ${this.startingChar}${this.fillProgress()}${this.fillSpace()}${this.endingChar} ${this.toMB(this.currentProgress)}/${this.toMB(this.total)} MB\n` +
                // ` ${Math.round(currentProgress / 1000)}KB/${Math.round(total / 1000)}KB ` +
