...
function enrichOptsData({ opt, validTargets, optValue, multiTargetSupport = true, filterFallback }) {
    ...
    return optValue
        ...
        if (optArr.indexOf(entry) !== entryIndex) {
            ...
            throw new index_js_1.default(`Duplicate target found for ${(0, ansi_colors_1.bold)(entry)}.`, {
                exit: 1,
        ...
        if (accumulator.some((res, idx) => {
            ...
            throw new index_js_1.default(`Target "${entrySet[0]}" is specified with and without a filter for option "${opt}".`, {
                exit: 1,
...
function validateTargets(target) {
    if ((0, option_1.getOptionValue)('except', false) && (0, option_1.getOptionValue)('only', false)) {
        throw new index_js_1.default('Cannot specify both --only and --except', {
            exit: 1,
            errorId: 'OPT-FILTER-4',
            arg: [(0, ansi_colors_1.bold)('--only'), (0, ansi_colors_1.bold)('--except')]
        });
    }
    const enrichedOnlyOpts = enrichOptsData({
        opt: 'only',
        optValue: (0, option_1.getOptionValue)('only'),
        validTargets: target === 'functions' ? [fnTarget] : onlyExceptTargets,
        filterFallback: target === 'functions' ? 'functions' : undefined
    });
    if (enrichedOnlyOpts.length > 0) {
        return enrichedOnlyOpts;
    }
    const enrichedExceptOpts = enrichOptsData({
        opt: 'except',
        optValue: (0, option_1.getOptionValue)('except'),
        validTargets: target === 'functions' ? [fnTarget] : onlyExceptTargets,
        filterFallback: target === 'functions' ? 'functions' : undefined
    });
...
function validatePorts() {
    ...
    const enrichedDebugOpts = enrichOptsData({
        opt: 'debug',
        optValue: (0, option_1.getOptionValue)('debug'),
        validTargets: portTargets
    });
    if (enrichedDebugOpts.some((opt) => {
        const noFilter = !opt.hasFilter;
        if (opt.target.target === 'client') {
            throw new index_js_1.default('Client cannot be debugged', {
                exit: 1,
    ...
    if (validTargetResultants.length === 0) {
        if (runtime_store_1.default.get('project.root') !== runtime_store_1.default.get('cwd')) {
            targets = targets.filter((target) => {
                ...
                const targetRoot = CONFIG[(target + 'Config')].source();
