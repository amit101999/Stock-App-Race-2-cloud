...
class API {
    constructor({ authNeeded = true, resolveOnError = false, maxRetry = 3, env = constants_1.DEFAULT.env_name, log = {}, headers = {}, isExternal = false, origin = constants_1.ORIGIN.admin, json = true, showWarning = true, envId = (0, project_1.getEnvId)(), printError = true } = {}) {
        ...
        this.requestOpts.headers = Object.assign(Object.assign({}, this.requestOpts.headers), { 'User-Agent': runtime_store_1.default.get('cli.package.name') + '/' + runtime_store_1.default.get('cli.package.version') });
    ...
    _logDownloadProgress({ chunk, error } = {}) {
        if (this.downloadProgress) {
            if (chunk) {
                return this.downloadProgress.tick(chunk.length);
            }
            if (error) {
                return this.downloadProgress.error(error);
            }
        }
    }
    _addAuthHeader() {
        return __awaiter(this, void 0, void 0, function* () {
            const credential = (yield Promise.resolve().then(() => __importStar(require('../authentication/credential.js')))).default;
    ...
    _parseResponse(resp, body, rawResponse) {
        ...
        if (rawResponseData && js_1.JS.includes(resp.headers['content-type'], 'json')) {
            try {
                ...
                (0, index_1.debug)('API Parsing Error: ' + err.message);
            }
        }
        if (resp.statusCode >= 400 && !this.logOpts.skipRespBody) {
            (0, index_1.debug)('Response Body : ' + JSON.stringify(resp.body) + '\n');
    ...
    _retry(err) {
        return __awaiter(this, void 0, void 0, function* () {
            if (++this.retryCount > this.maxRetryCount) {
                throw new index_js_1.default('API Error after retrying : ' + err.message, {
                    original: err,
                    exit: 2
                });
            }
                        ...
                        switch (err.code) {
                            ...
                            case 'ESOCKETTIMEDOUT':
                                ...
                                try {
                                    ...
                                    const error = index_js_1.default.getErrorInstance(e);
                                    error.exit = 2;
                                    reject(error);
                                }
                                break;
                        }
                        reject(new index_js_1.default('API Error: ' + err.message, {
                            original: err,
                            exit: 2
                        }));
                    }
