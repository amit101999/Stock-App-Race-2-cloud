...
const bioHandler = async (req, res) => {
	return (await import('./bio.js')).default(index, targetName, type)(req, res);
};

function getHandler() {
	switch (type) {
		case 'browserlogic': {
			return browserLogicHandler;
		}
		case 'aio': {
			return aioHandler;
		}
...
const server = createServer(getHandler());
