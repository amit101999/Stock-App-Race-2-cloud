...
class Archiver {
    ...
    load(content, { createFolders = true } = {}) {
        if (!Buffer.isBuffer(content) && !js_1.JS.isString(content)) {
            throw new index_js_1.default('Unable to parse zip content, content should be either a string or buffer', { exit: 2 });
        }
    ...
    getFileMode(file) {
        ...
        try {
            ...
            (0, index_1.debug)('Zip file mode error: ', er);
        }
        return;
    }
    _writeFile(file, to) {
    ...
    extract(to, from = '/', { isFolder = true, recursive = false, ignoreLevel = 0 } = {}) {
        const extractPromise = this.jszip.then((zip) => __awaiter(this, void 0, void 0, function* () {
            if (isFolder) {
                ...
                if (!folder) {
                    throw new index_js_1.default('Unable to extract zip, FolderObj is null', {
                        exit: 2
                    });
                }
            ...
            if (fromFileObj.length === 0) {
                throw new index_js_1.default('Unable to extract zip, FileObj is null', {
                    exit: 2
                });
            }
