...
exports.default = (_fns) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const fnDirPath = config_1.functionsConfig.source();
    const fnArr = (yield fnAPI.getAllFunctions());
    if (!Array.isArray(fnArr) || fnArr.length === 0) {
        (0, index_1.info)('No function found in remote console.');
        return;
    }
    ...
    targets = targets.filter((target) => {
        ...
        if (!target.valid &&
            ...
            (0, index_1.debug)('Invalid target : ' + target.name + ' reason : ' + target.failure_reason);
            (0, index_1.warning)(`skipping pull of function(${target.name}) since the package is invalid. \nContact catalyst support with debug log.`);
            new error_1.default('invalid package: ' + target.name, { exit: 2 });
        }
        else if (!target.valid &&
            ((_d = target.failure_reason) === null || _d === void 0 ? void 0 : _d.toString().startsWith('unable to locate python'))) {
            (0, index_1.warning)(`skipping pull of function(${target.name}) since ${target.failure_reason}`);
        }
        (_g = (_f = (_e = target.additionalInfo) === null || _e === void 0 ? void 0 : _e.compilerOut) === null || _f === void 0 ? void 0 : _f.error) === null || _g === void 0 ? void 0 : _g.forEach((error) => {
            console.log(`${(0, ansi_colors_1.red)(logger_1.CHAR.error)} Error while compiling function[${target.name}]`);
            console.log(error);
        });
        (_k = (_j = (_h = target.additionalInfo) === null || _h === void 0 ? void 0 : _h.compilerOut) === null || _j === void 0 ? void 0 : _j.warning) === null || _k === void 0 ? void 0 : _k.forEach((warn) => {
            console.warn(`${(0, ansi_colors_1.yellow)(logger_1.CHAR.warning)} Warning while compiling function[${target.name}]`);
            console.warn(warn);
        });
        return target.valid;
    });
    if (isFnPulled) {
        runtime_store_1.default.set('context.payload.functions.targets', targets);
        ...
        if (isFnPulledFailed) {
            ...
            runtime_store_1.default.set('context.payload.functions.status', 2);
        }
    }
});
