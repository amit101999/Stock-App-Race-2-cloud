'use strict';
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const env_js_1 = require("../../util_modules/env.js");
const dc_js_1 = require("../../util_modules/dc.js");
const dc_type_js_1 = __importDefault(require("../../util_modules/constants/lib/dc-type.js"));
class AUTH {
    static get web() {
        const dc = (0, dc_js_1.getActiveDCType)();
        if (dc === undefined) {
            return AUTH._web[dc_type_js_1.default.us.ref];
        }
        return AUTH._web[dc.w_auth];
    }
    static get mobile() {
        const dc = (0, dc_js_1.getActiveDCType)();
        if (dc === undefined) {
            return AUTH._mobile[dc_type_js_1.default.us.ref];
        }
        return AUTH._mobile[dc.m_auth];
    }
}
exports.default = AUTH;
AUTH._web = {
    [dc_type_js_1.default.us.ref]: {
        id: (0, env_js_1.envOverride)('W_CATALYST_CLIENT_ID', '1000.D5IIHDXSPN2MII26AD0V61I6RMVSNM'),
        secret: (0, env_js_1.envOverride)('W_CATALYST_CLIENT_SECRET', '02ee875ecfc50573e5cc8d62916ad3077be20d0f42')
    }
};
AUTH._mobile = {
    [dc_type_js_1.default.us.ref]: {
        id: (0, env_js_1.envOverride)('M_CATALYST_CLIENT_ID', '1004.NMUISG5YKILERY9G29LJHZWIY9II7Y'),
        secret: (0, env_js_1.envOverride)('M_CATALYST_CLIENT_SECRET', 'ee359029dc211b37c797d3584c59953f2bd6aa1adc')
    }
};
