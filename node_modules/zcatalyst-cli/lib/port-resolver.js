        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (isNaN(port)) {
                if (type === 'debug' && feature === 'client') {
                    throw new index_js_1.default('client cannot be debugged', { exit: 2 });
                }
                if (!fallbackNeeded) {
                    return -1;
                }
                if (feature === 'appsail') {
                    if (!server || type !== 'http') {
                        throw new index_js_1.default('Invalid appsail details', { exit: 2 });
                    }
                    port = constants_1.DEFAULT.serve_port['http']['appsail'][server];
                }
                else if (feature === 'slate') {
                    if (!server || type !== 'http') {
                        throw new index_js_1.default('Invalid slate details', { exit: 2 });
                    }
            ...
            if (port === -1) {
                throw new index_js_1.default('port resolved to -1 for feature ' + feature + ' and type ' + type, {
                    exit: 2
                });
            }
            let finalPort = port;
            ...
            if (finalPort !== port) {
                (0, index_1.debug)(`${type} port : '${port}' is not available. Hence port '${finalPort}' will be used instead.`);
            }
            return finalPort;
        });
    }
