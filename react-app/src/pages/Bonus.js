...
const Bonus = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log('Bonus form submitted', form);
    ...
    // Show the Bonus Allocation section after save
    setIsFormSaved(true);
  };
  ...
  const handlePageChange = (newPage) => {
    if (newPage >= 1 && newPage <= totalPages) {
      ...
      const tableCard = document.querySelector('.bonus-table-card');
      if (tableCard) {
  ...
  return (
    <div className="bonus-page">
      ...
      {isFormSaved && (
        <div className="bonus-table-card">
        <h2>Bonus Allocation</h2>
          ...
          <div className="table-rows">
            {paginatedRows.length > 0 ? (
              paginatedRows.map((row, idx) => {
                ...
                // Calculate Bonus Qty: (Current Qty / Share Unit) * Allotted Unit
                ...
                const bonusQty = shareUnit > 0 
                  ? Math.floor((currentQty / shareUnit) * allottedUnit)
                  : 0;
                // Calculate New Qty: Current Qty + Bonus Qty
                const newQty = currentQty + bonusQty;
